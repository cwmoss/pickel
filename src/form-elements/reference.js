...
export default class Reference extends Face {
    ...
    connectedCallback() {
        ...
        //await
        console.log("$REF connected", this.name, this.value);
        // this.fetch_preview();
    }
    ...
    async fetch_preview() {
        console.log("$REF doc-preview", this.name, this.value);
        // TODO: create new reference
        if (this.value == "") return;
        let doc = await api.document_preview(this.value._ref);
        console.log("$REF doc-preview", this.name, this.value, doc);
    ...
    get allowed_type() {
        console.log("$REF", this.name, this.schema);

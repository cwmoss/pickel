...
OrderedMap.prototype = {
	...
	get: function(key) {
		...
		return found$1 == -1 ? undefined : this.content[found$1 + 1];
	},
...
function nfa(expr) {
	...
	function compile(expr$1, from) {
		if (expr$1.type == "choice") return expr$1.exprs.reduce((out, expr$2) => out.concat(compile(expr$2, from)), []);
			...
			return [edge(cur)];
		} else if (expr$1.type == "name") return [edge(from, undefined, expr$1.value)];
	...
	static schemaRules(schema) {
		...
		function insert(rule) {
			...
			for (; i$1 < result.length; i$1++) {
				let next = result[i$1], nextPriority = next.priority == null ? 50 : next.priority;
	...
	findAround(parent, content, before) {
		if (parent != content && this.find) {
			for (let i$1 = 0; i$1 < this.find.length; i$1++) if (this.find[i$1].pos == null && parent.nodeType == 1 && parent.contains(this.find[i$1].node)) {
				let pos = content.compareDocumentPosition(this.find[i$1].node);
				if (pos & (before ? 2 : 4)) this.find[i$1].pos = this.currentPos;
			}
		}
	}
	findInText(textNode) {
	...
	*/
	touches(pos, recover) {
		...
		for (let i$1 = 0; i$1 < this.ranges.length; i$1 += 3) {
			let start = this.ranges[i$1] - (this.inverted ? diff : 0);
			if (start > pos) break;
			let oldSize = this.ranges[i$1 + oldIndex], end = start + oldSize;
			if (pos <= end && i$1 == index * 3) return true;
	...
	*/
	appendMapping(mapping) {
		for (let i$1 = 0, startSize = this._maps.length; i$1 < mapping._maps.length; i$1++) {
			...
			this.appendMap(mapping._maps[i$1], mirr != null && mirr < i$1 ? startSize + mirr : undefined);
		}
	}
	/**
	...
	*/
	appendMappingInverted(mapping) {
		for (let i$1 = mapping.maps.length - 1, totalSize = this._maps.length + mapping._maps.length; i$1 >= 0; i$1--) {
			let mirr = mapping.getMirror(i$1);
			this.appendMap(mapping._maps[i$1].invert(), mirr != null && mirr > i$1 ? totalSize - mirr - 1 : undefined);
		}
	}
	/**
	...
	*/
	_map(pos, assoc, simple) {
		...
		for (let i$1 = this.from; i$1 < this.to; i$1++) {
			let map = this._maps[i$1], result = map.mapResult(pos, assoc);
...
function removeMark(tr$1, from, to, mark) {
	...
	matched.forEach((m) => tr$1.step(new RemoveMarkStep(m.from, m.to, m.style)));
}
function clearIncompatible(tr$1, pos, parentType, match = parentType.contentMatch, clearNewlines = true) {
	...
	for (let i$1 = 0; i$1 < node.childCount; i$1++) {
		...
		if (!allowed) replSteps.push(new ReplaceStep(cur, end, Slice.empty));
			...
			match = allowed;
			for (let j = 0; j < child.marks.length; j++) if (!parentType.allowsMarkType(child.marks[j].type)) tr$1.step(new RemoveMarkStep(cur, end, child.marks[j]));
...
function setBlockType$1(tr$1, from, to, type, attrs) {
	...
	tr$1.doc.nodesBetween(from, to, (node, pos) => {
		...
		if (node.isTextblock && !node.hasMarkup(type, attrsHere) && canChangeType(tr$1.doc, tr$1.mapping.slice(mapFrom).map(pos), type)) {
			...
			clearIncompatible(tr$1, tr$1.mapping.slice(mapFrom).map(pos, 1), type, undefined, convertNewlines === null);
...
function replaceNewlines(tr$1, node, pos, mapFrom) {
	node.forEach((child, offset) => {
		if (child.isText) {
			let m, newline = /\r?\n|\r/g;
			while (m = newline.exec(child.text)) {
				let start = tr$1.mapping.slice(mapFrom).map(pos + 1 + offset + m.index);
	...
	*/
	static between($anchor, $head, bias) {
		...
		if (!$head.parent.inlineContent) {
			let found$1 = Selection$1.findFrom($head, bias, true) || Selection$1.findFrom($head, -bias, true);
	...
	*/
	...
	replace(tr$1, content = Slice.empty) {
		if (content == Slice.empty) {
			...
			let sel = Selection$1.atStart(tr$1.doc);
...
function selectionToInsertionEnd$1(tr$1, startLen, bias) {
	...
	let step = tr$1.steps[last];
	if (!(step instanceof ReplaceStep || step instanceof ReplaceAroundStep)) return;
	let map = tr$1.mapping.maps[last], end;
	map.forEach((_from, _to, _newFrom, newTo) => {
		if (end == null) end = newTo;
	});
	tr$1.setSelection(Selection$1.near(tr$1.doc.resolve(end), bias));
}
	...
	if ($cursor.parent.content.size == 0 && (textblockAt(before, "end") || NodeSelection.isSelectable(before))) for (let depth = $cursor.depth;; depth--) {
		...
		if (delStep && delStep.slice.size < delStep.to - delStep.from) {
			if (dispatch) {
				...
				tr$1.setSelection(textblockAt(before, "end") ? Selection$1.findFrom(tr$1.doc.resolve(tr$1.mapping.map($cut.pos, -1)), -1) : NodeSelection.create(tr$1.doc, $cut.pos - before.nodeSize));
...
const joinForward$1 = (state, dispatch, view) => {
	...
	if ($cursor.parent.content.size == 0 && (textblockAt(after, "start") || NodeSelection.isSelectable(after))) {
		...
		if (delStep && delStep.slice.size < delStep.to - delStep.from) {
			if (dispatch) {
				let tr$1 = state.tr.step(delStep);
				tr$1.setSelection(textblockAt(after, "start") ? Selection$1.findFrom(tr$1.doc.resolve(tr$1.mapping.map($cut.pos)), 1) : NodeSelection.create(tr$1.doc, tr$1.mapping.map($cut.pos)));
				dispatch(tr$1.scrollIntoView());
			}
			return true;
		}
...
const exitCode$1 = (state, dispatch) => {
	...
	if (dispatch) {
		...
		tr$1.setSelection(Selection$1.near(tr$1.doc.resolve(pos), 1));
		dispatch(tr$1.scrollIntoView());
	}
	return true;
};
	...
	if (canDelAfter && textblockAt(after, "start", true) && textblockAt(before, "end")) {
		...
		if (at.canReplace(at.childCount, at.childCount, afterText.content)) {
			if (dispatch) {
				...
				let tr$1 = state.tr.step(new ReplaceAroundStep($cut.pos - wrap$1.length, $cut.pos + after.nodeSize, $cut.pos + afterDepth, $cut.pos + after.nodeSize - afterDepth, new Slice(end, wrap$1.length, 0), 0, true));
				dispatch(tr$1.scrollIntoView());
			}
			return true;
		}
...
const mac$3 = typeof navigator != "undefined" ? /Mac|iP(hone|[oa]d)/.test(navigator.platform) : typeof os != "undefined" && os.platform ? os.platform() == "darwin" : false;
...
function docViewDesc(doc$2, outerDeco, innerDeco, dom, view) {
	...
	let docView = new NodeViewDesc(undefined, doc$2, outerDeco, innerDeco, dom, dom, dom, view, 0);
...
function preMatch(frag, parentDesc) {
	...
	let fI = frag.childCount, matched = new Map(), matches$1 = [];
	outer: while (fI > 0) {
		let desc;
		for (;;) if (descI) {
...
function addToSibling(wrap$1, lastWrap, node, sibling, depth) {
	if (depth < wrap$1.length && depth < lastWrap.length && wrap$1[depth] == lastWrap[depth]) {
		...
		if (match.matchType(depth == wrap$1.length - 1 ? node.type : wrap$1[depth + 1])) return sibling.copy(sibling.content.append(Fragment.from(withWrappers(node, wrap$1, depth + 1))));
	}
}
...
function mapChildren(oldChildren, newLocal, mapping, node, offset, oldOffset, options) {
	...
	for (let i$1 = 0; i$1 < children.length; i$1 += 3) if (children[i$1 + 1] < 0) {
		...
		let to = mapping.map(oldChildren[i$1 + 1] + oldOffset, -1), toLocal = to - offset;
...
function readDOMChange(view, from, to, typeOver, addedNodes) {
	...
	let mkTr = (base$1) => {
		...
		if (parse.sel) {
			...
			if (sel$1 && !(chrome && view.composing && sel$1.empty && (change.start != change.endB || view.input.lastChromeDelete < Date.now() - 100) && (sel$1.head == chFrom || sel$1.head == tr$1.mapping.map(chTo) - 1) || ie$1 && sel$1.empty && sel$1.head == chFrom)) tr$1.setSelection(sel$1);
		}
		if (compositionID) tr$1.setMeta("composition", compositionID);
	...
	*/
	...
	updateDraggedNode(dragging, prev) {
		...
		if (this.state.doc.nodeAt(sel.from) == sel.node) found$1 = sel.from;
			...
			if (moved == sel.node) found$1 = movedPos;
		}
...
function resolveFocusPosition(doc$2, position = null) {
	...
	const selectionAtStart = Selection$1.atStart(doc$2);
	const selectionAtEnd = Selection$1.atEnd(doc$2);
...
function createNodeFromContent(content, schema, options) {
	...
	if (isJSONContent) try {
		...
		if (isArrayContent) return Fragment.fromArray(content.map((item) => schema.nodeFromJSON(item)));
...
var keyboardShortcut = (name) => ({ editor, view, tr: tr$1, dispatch }) => {
	...
	capturedTransaction == null ? void 0 : capturedTransaction.steps.forEach((step) => {
		const newStep = step.map(tr$1.mapping);
...
function simplifyChangedRanges(changes) {
	...
	return uniqueChanges.length === 1 ? uniqueChanges : uniqueChanges.filter((change, index) => {
		const rest = uniqueChanges.filter((_, i$1) => i$1 !== index);
...
var joinListBackwards = (tr$1, listType) => {
	...
	const canJoinBackwards = list.node.type === (nodeBefore == null ? void 0 : nodeBefore.type) && canJoin(tr$1.doc, list.pos);
...
var joinListForwards = (tr$1, listType) => {
	...
	const canJoinForwards = list.node.type === (nodeAfter == null ? void 0 : nodeAfter.type) && canJoin(tr$1.doc, after);
...
var toggleList = (listTypeOrName, itemTypeOrName, keepMarks, attributes = {}) => ({ editor, tr: tr$1, state, dispatch, chain, commands, can }) => {
	...
	if (range.depth >= 1 && parentList && range.depth - parentList.depth <= 1) {
		...
		if (isList(parentList.node.type.name, extensions) && listType.validContent(parentList.node.content) && dispatch) return chain().command(() => {
			tr$1.setNodeMarkup(parentList.pos, listType);
			return true;
...
var toggleMark = (typeOrName, attributes = {}, options = {}) => ({ state, commands }) => {
	...
	if (isActive2) return commands.unsetMark(type, { extendEmptyMarkRange });
...
var toggleWrap = (typeOrName, attributes = {}) => ({ state, commands }) => {
	const type = getNodeType(typeOrName, state.schema);
	const isActive2 = isNodeActive(state, type, attributes);
	if (isActive2) return commands.lift(type);
	...
	for (let i$1 = 0; i$1 < plugins.length; i$1 += 1) {
		...
		if (plugin.spec.isInputRules && (undoable = plugin.getState(state))) {
			if (dispatch) {
				...
				for (let j = toUndo.steps.length - 1; j >= 0; j -= 1) tr$1.step(toUndo.steps[j].invert(toUndo.docs[j]));
				if (undoable.text) {
...
function run$2(config) {
	...
	rules.forEach((rule) => {
		...
		if (handler === null || !tr$1.steps.length) return;
	...
	const plugin = new Plugin({
		state: {
			...
			apply(tr$1, prev, state) {
				...
				if (isSimulatedInput) setTimeout(() => {
...
function run2(config) {
	...
	state.doc.nodesBetween(from, to, (node, pos) => {
		...
		const matches$1 = pasteRuleMatcherHandler(textToMatch, rule.find, pasteEvent);
	...
	const success = handlers$1.every((handler) => handler !== null);
	return success;
}
...
function pasteRulesPlugin(props) {
	...
	const processEvent = ({ state, from, to, rule, pasteEvt }) => {
		...
		if (!handler || !tr$1.steps.length) return;
		try {
			dropEvent = typeof DragEvent !== "undefined" ? new DragEvent("drop") : null;
		} catch {
			dropEvent = null;
	...
	addProseMirrorPlugins() {
		return [new Plugin({
			...
			props: { clipboardTextSerializer: () => {
				...
				return getTextBetween(doc$2, range, {
	...
	addKeyboardShortcuts() {
		const handleBackspace$1 = () => this.editor.commands.first(({ commands }) => [
			...
			() => commands.command(({ tr: tr$1 }) => {
				...
				const isAtStart = parentIsIsolating && $parentPos.parent.childCount === 1 ? parentPos === $anchor.pos : Selection$1.atStart(doc$2).from === pos;
	...
	addProseMirrorPlugins() {
		return [new Plugin({
			...
			appendTransaction: (transactions, oldState, newState) => {
				...
				if (empty$1 || !allWasSelected) return;
	...
	querySelectorAll(selector, attributes = {}, firstItemOnly = false) {
		...
		this.children.forEach((childPos) => {
			...
			if (childPos.node.type.name === selector) {
				...
				if (doesAllAttributesMatch) nodes.push(childPos);
			}
			if (firstItemOnly && nodes.length > 0) return;
...
var Editor = class extends EventEmitter {
	...
	*/
	unregisterPlugin(nameOrPluginKeyToRemove) {
		...
		if (prevPlugins.length === plugins.length) return void 0;
		const state = this.state.reconfigure({ plugins });
		this.view.updateState(state);
		return state;
	}
...
function wrappingInputRule(config) {
	return new InputRule({
		...
		handler: ({ state, range, match, chain }) => {
			...
			const $start = tr$1.doc.resolve(range.from);
	...
	return {
		...
		markdownTokenizer: {
			...
			tokenize(src, _tokens, _lexer) {
				...
				if (missingRequired) return void 0;
				return {
					type: nodeName,
					raw: match[0],
					attributes
...
`;
		...
		while (i$1 < lines.length) {
			...
			if (nextLine.trim() === "") {
				...
				const nextNonEmpty = lines[i$1 + 1 + nextNonEmptyIndex];
...
var Blockquote = Node3.create({
	...
	renderMarkdown: (node, h$1) => {
		...
		node.content.forEach((child) => {
			const childContent = h$1.renderChildren([child]);
		...
		return {
			...
			"Shift-Tab": ({ editor }) => {
				...
				if (empty$1) return editor.commands.command(({ tr: tr$1 }) => {
					...
					if (cursorPosInLine <= spacesToRemove) tr$1.setSelection(TextSelection.create(tr$1.doc, lineStartPos));
					return true;
				});
				return editor.commands.command(({ tr: tr$1 }) => {
					const { from, to } = selection;
			...
			ArrowDown: ({ editor }) => {
				...
				const nodeAfter = doc$2.nodeAt(after);
				if (nodeAfter) return editor.commands.command(({ tr: tr$1 }) => {
					tr$1.setSelection(Selection$1.near(doc$2.resolve(after)));
					return true;
				});
...
function init$2(customSchemes = []) {
	...
	for (let i$1 = 0; i$1 < customSchemes.length; i$1++) {
		...
		const optionalSlashSlash = customSchemes[i$1][1];
...
var ListItem = Node3.create({
	...
	parseMarkdown: (token, helpers) => {
		...
		if (token.tokens && token.tokens.length > 0) {
			...
			if (hasParagraphTokens) content = helpers.parseChildren(token.tokens);
else {
...
var OrderedList = Node3.create({
	...
	markdownTokenizer: {
		...
		tokenize: (src, _tokens, lexer) => {
			...
			if (listItems.length === 0) return void 0;
...
var TaskItem = Node3.create({
	...
	renderMarkdown: (node, h$1) => {
		...
		return renderNestedMarkdownContent(node, h$1, prefix);
	},
	addKeyboardShortcuts() {
		const shortcuts = {
			Enter: () => this.editor.commands.splitListItem(this.name),
...
RopeSequence.prototype.get = function get(i$1) {
	if (i$1 < 0 || i$1 >= this.length) return undefined;
...
var Leaf = function(RopeSequence$1) {
...
var Append = function(RopeSequence$1) {
...
var Branch = class Branch {
	...
	addTransform(transform, selection, histOptions, preserveItems) {
		...
		for (let i$1 = 0; i$1 < transform.steps.length; i$1++) {
			let step = transform.steps[i$1].invert(transform.docs[i$1]);
			let item = new Item(transform.mapping.maps[i$1], step, selection), merged;
	...
	remapping(from, to) {
		...
		this.items.forEach((item, i$1) => {
			let mirrorPos = item.mirrorOffset != null && i$1 - item.mirrorOffset >= from ? maps.maps.length - item.mirrorOffset : undefined;
...
function applyTransaction(history$1, state, tr$1, options) {
	...
	if (tr$1.steps.length == 0) return history$1;
else if (appended && appended.getMeta(historyKey)) if (appended.getMeta(historyKey).redo) return new HistoryState(history$1.done.addTransform(tr$1, undefined, options, mustPreserveItems(state)), history$1.undone, rangesFor(tr$1.mapping.maps), history$1.prevTime, history$1.prevComposition);
else return new HistoryState(history$1.done, history$1.undone.addTransform(tr$1, undefined, options, mustPreserveItems(state)), null, history$1.prevTime, history$1.prevComposition);
		...
		return new HistoryState(history$1.done.addTransform(tr$1, newGroup ? state.selection.getBookmark() : undefined, options, mustPreserveItems(state)), Branch.empty, prevRanges, tr$1.time, composition == null ? history$1.prevComposition : composition);
...
//#endregion
export { Editor, Extension, Node3 as Node, index_default as StarterKit };